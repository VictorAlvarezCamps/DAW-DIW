...
function mostrarUbicacion(){
    ...
    coord1 = parseFloat(fallas.features[this.dataset.num].geometry.coordinates[0]);
    coord2 = parseFloat(fallas.features[this.dataset.num].geometry.coordinates[1]);
    ...
    //Añadimos coordenadas al mapa
    var layer = new ol.layer.Vector({
        source: new ol.source.Vector({
            features: [
                new ol.Feature({
                    geometry: new ol.geom.Point(ol.proj.fromLonLat([coord1, coord2]))
                })
            ]
        })
    });
